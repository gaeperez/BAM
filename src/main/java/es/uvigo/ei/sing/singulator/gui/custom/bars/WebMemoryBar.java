...
public class WebMemoryBar extends WebLabel implements ShapeProvider, SizeMethods<WebLabel> {
	...
	private ImageIcon memoryIcon = Functions.loadIcon(Constants.ICON_MEMORY_16);
	...
	private boolean showMaximumMemory = false;
	...
	protected String getMemoryBarTooltipText() {
		return "<html>" + LanguageManager.get("weblaf.ex.memorybar.used") + " <b>"
				+ FileUtils.getFileSizeString(usedMemory, getDigits(usedMemory)) + "</b> "
				+ LanguageManager.get("weblaf.ex.memorybar.alloc") + " <b>"
				+ FileUtils.getFileSizeString(allocatedMemory, getDigits(allocatedMemory)) + "</b> " + getMaximumText()
				+ "</b></html>";
	}
	...
	private void paintInColors(Graphics2D g2d) {
		double res = usedMemory * 100 / allocatedMemory;
	...
	private int getProgressWidth(final long progress, final boolean fill) {
		return Math.round((float) (getWidth() - (drawBorder ? 4 + shadeWidth * 2 : 2) - (fill ? 0 : 1)) * progress
				/ (showMaximumMemory ? maxMemory : allocatedMemory));
	}
